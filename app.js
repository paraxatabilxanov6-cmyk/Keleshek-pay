const KEY="keleshek_pay_v4";
const state={students:[],payments:[],tab:"home",month:new Date().toISOString().slice(0,7)};
const $=s=>document.querySelector(s);
function load(){try{const x=JSON.parse(localStorage.getItem(KEY)||"null");if(x){state.students=x.students||[];state.payments=x.payments||[]}}catch(e){console.warn(e)}}
function save(){localStorage.setItem(KEY,JSON.stringify({students:state.students,payments:state.payments,version:4,savedAt:new Date().toISOString()}))}
